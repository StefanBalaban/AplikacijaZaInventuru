...
namespace Generator.Helpers
{
    ...
    {
        public Dictionary<string, string> EndpointMethods = new()
        {
            { "Create", "Post" },
            { "Update", "Put" },
            { "Delete", "Delete" },
                    ...
                    Parameter(Identifier("request")).WithAttributeLists(
                            SingletonList(
                                AttributeList(
                                    SingletonSeparatedList(
                                        Attribute(
                                            IdentifierName("FromRoute"))))))
                        .WithType(IdentifierName($"{endpoint}{modelClassName}Request"));
        ...
        public SyntaxList<MemberDeclarationSyntax> GenerateRequestOrDtoClassMemeberProperties(string endpoint,
            List<PropertiesWithInfo> propertiesWithInfo, string modelClassName = null,
            List<AttributesWithInfo> attributesWithInfo = null,
            ArgumentFilterConstantsHelpers argumentFilterConstantsHelpers = null)
        {
            var list = new List<MemberDeclarationSyntax>();

            if (endpoint.Equals("Update") || endpoint.Equals("Dto"))
            ...
            if (endpoint.Equals("Create") || endpoint.Equals("Update") || endpoint.Equals("Dto"))
                propertiesWithInfo.Where(x => x.Method.Equals(EndpointMethods[endpoint])).ToList().ForEach(x =>
                    list.Add(
                        PropertyDeclaration(
            ...
            if (endpoint.Equals("ListPaged"))
            {
                ...
                attributesWithInfo.ForEach(x =>
                    x.Arguments.Where(y => y.Any() && !y.Contains("INCLUDE")).ToList().ForEach(y => list.Add(
                        PropertyDeclaration(
                                NullableType(IdentifierName(x.Type)),
                                Identifier(
                                    $"{x.PropertyIdentifier}{argumentFilterConstantsHelpers.GetArgumentSufix(y)}"))
                            .WithModifiers(
                                TokenList(
                                    Token(SyntaxKind.PublicKeyword)))
                            .WithAccessorList(
        ...
        public IEnumerable<ExpressionSyntax> GenerateServiceRequestObjectInitialization(string endpoint,
            List<KeyValuePair<string, string>> propertiesWithAttributes)
        {
            var properties = propertiesWithAttributes.Where(x => x.Value == EndpointMethods[endpoint])
        ...
        internal SeparatedSyntaxList<ArgumentSyntax> GenerateListPagedSpecificationRequest(
            List<AttributesWithInfo> attributesWithInfo, ArgumentFilterConstantsHelpers argumentFilterConstantsHelpers,
            bool pagedSpec = false)
        {
            ...
            // Remove last comma
            if (arguments.Count > 0) arguments.RemoveAt(arguments.Count - 1);
